...
// init pagination
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            $product_image = "download.png";
            if (!$this->upload->do_upload()) {
                $errors = array('error' => $this->upload->display_errors());
                $this->session->set_flashdata('product_created', 'Greska prilikom upisivanja u bazu');
            } else {
    ...
    public function delete()
    {
        ...
        if (isset($_POST['zabrisanje'])) {
            unlink("./assets/img/productimg/" . $_POST['zabrisanje']);
            // redirect('products');
        } 
        if (isset($_POST['id'])){
            $this->product_model->delete_product($_POST['id']);
    ...
    public function update()
    {
        ...
        if (!$this->upload->do_upload()) {
            $errors = array('error' => $this->upload->display_errors());
            $product_image = 'download.png';
            $this->session->set_flashdata('product_updated', 'GRESKA pri uploadu');
        } else {
            $data = array('upload_data' => $this->upload->data());
            $product_image = $_FILES['userfile']['name'];
        }
    ...
    public function update_pdf()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            $this->product_model->update_pdf($pdf);
            $this->session->set_flashdata('product_updated', 'Proizvod uspesno izmenjen');
            unlink("./assets/pdf/" . $_POST['zapdf']);
            redirect('products');
        }
    }
    ...
    public function downloadFile($filename)
    {
        ...
        $data = file_get_contents("./assets/pdf/".$filename); // Read the file's contents
